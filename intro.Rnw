...
\usepackage[hyperref,colorlinks=true,urlcolor=blue,pdfpagelabels]{hyperref}
\usepackage{ctable}
\usepackage{amsmath}
\usepackage{Sweave}
\usepackage{pifont}
...
\definecolor{grisfonce}{gray}{0.50}
...
\DefineVerbatimEnvironment{Sinput}{Verbatim}{formatcom=\color{inputcol},xleftmargin=\parindent}
\DefineVerbatimEnvironment{Soutput}{Verbatim}{fontshape=sl,formatcom=\color{outputcol},xleftmargin=2ex,xleftmargin=\parindent}
\DefineVerbatimEnvironment{Scode}{Verbatim}{formatcom=\color{inputcol},xleftmargin=\parindent}
%\newenvironment{Schunk}{}{}
\DefineVerbatimEnvironment{CodeInput}{Verbatim}{}
\DefineVerbatimEnvironment{CodeOutput}{Verbatim}{fontshpae=sl}
%\newenvironment{CodeChunk}{}{}

\setkeys{Gin}{width=0.8\textwidth}

\fvset{listparameters={\setlength{\topsep}{0pt}\setlength{\parskip}{0pt}}}
...
%\CustomVerbatimEnvironment{code}{Verbatim}{numbers=left,frame=single,framesep=8pt,framerule=1pt}
\CustomVerbatimEnvironment{code}{Verbatim}{numbers=left}
...
%\setmarginsrb{2cm}{1.5cm}{2cm}{2cm}{0.5cm}{0.5cm}{0.5cm}{0.5cm}
%\setcounter{tocdepth}{2}
%\definecolor{grisclair}{gray}{0.97}

\title{\includegraphics[width=1.4cm]{img/Rlogo.jpg} pour les sociologues\\(et assimilés)}
\author{Julien Barnier\\ Groupe de Recherche sur la Socialisation\\
  CNRS -- UMR 5040\\ \href{mailto:julien.barnier@ens-lyon.fr}{\texttt{julien.barnier@ens-lyon.fr}}}
\date{\today{}}
%\abstract{}
...
<<echo=FALSE>>=
options(prompt='R> ', width=75, max.print=70)
...
<<Rversion,results=tex,echo=FALSE>>=
...
\url{http://alea.fr.eu.org/j/intro_R.html}

Ce document est diffusé sous licence \textit{Creative Commons
  Paternité -- Non commercial}~:

\url{http://creativecommons.org/licenses/by-nc/2.0/fr/}

L'auteur tient à remercier Mayeul Kauffmann et Julien Biaudet pour
leurs corrections et suggestions.
...
\textsf{SAS}, \textsf{Linux}, \textsf{rgrs}, \textsf{ade4}\ldots). Les
...
Lorsqu'on présente des commandes saisies sous \R et leur résultat, la
commande saisie est indiquée avec une police à chasse fixe bleu
foncé précédée de l'invite de commande \texttt{R>}~:

<<eval=FALSE>>=
summary(rnorm(100))
@ %

Le résultat de la commande tel qu'affiché par \R est indiqué dans une
police à chasse fixe inclinée rouge foncé~:
...
statistique dérivé du langage \textsf{S}. Il est développé depuis plus
de 10 ans par un groupe de volontaires de différents pays. C'est un
...
\footnotetext{Il en existe actuellement plus de 1500, disponibles sur le
...
\item par son mode de fonctionnement, \R charge normalement
  l'intégralité des données traitées en mémoire. Il nécessite donc une
  machine relativement puissante pour travailler sur des grosses
  enquêtes de plusieurs milliers d'individus~;
\item il n'existe pas encore d'interface graphique pour \R équivalente
  à celle d'autres logiciels comme \textsf{SPSS} ou
  \textsf{Modalisa}. \R fonctionne à l'aide de scripts (des petits
  programmes) édités et exécutés au fur et à mesure de l'analyse, et
  se rapprocherait davantage de \textsf{SAS} dans son utilisation
  (mais avec une syntaxe et une philosophie très différentes).
\end{itemize}

À noter que ce dernier point, qui peut apparaître comme un gros
handicap, s'avère après un temps d'apprentissage être un mode
d'utilisation d'une grande souplesse.
...
\begin{figure}
  \begin{center}
    \includegraphics{img/scr_RGui.png}
  \end{center}
...
\begin{astuce}
  ...
  \textsf{Windows}. Sous \textsf{Linunx} on utilise le traditionnel
  \texttt{Control + C}.
\end{astuce}
...
18ème élément du vecteur, tandis que le 182 de la troisième ligne est
à la 35ème position.
...
Ici, la fonction \rfunc{length} nous renvoit le nombre d'éléments du
...
Et oui, par défaut, \R renvoit \texttt{NA} pour un grand nombre de
...
\begin{Scode}[label={Exemple de commentaire}]
...
l'extension \rgrs.\margrs{} Cette extension et son
installation sont décrites dans la partie~\ref{sec_rgrs},
page~\pageref{sec_rgrs}\footnote{À noter que les exemples de ce
  document ne pourront être reproduits qu'avec une version de \rgrs
  supérieure à 0.2-11.}.
...
\rgrs (après l'avoir installée, bien entendu)~:
...
La tableau précédent nous indique que parmi nos enquêtés on trouve 894
hommes et 1106 femmes.
...
non-réponses du tableau résultat. L'utilisation de \rfunc{summary}
permet l'affichage du tri à plat et du nombre de non-réponses~:
...
utiliser la fonction \rfunc{freq} de \margrs l'extension \texttt{rgrs}.
...
diagrammes de Cleveland, obtenus avec la fonction \rfunc{dotchart}. On
doit appliquer cette fonction au tri à plat de la variable, obtenu
avec la fonction \rfunc{table}. Le résultat se trouve
...
\textit{répertoire de travail} de \R. De quoi s'agit-il~? Tout
simplement du répertoire dans lequel \R est actuellement en train de
s'exécuter. Pour savoir quel est le répertoire de travail actuel, on
peut utiliser la fonction \rfunc{getwd}\footnote{Le résultat indiqué
  ici correspond à système \textsf{Linux}, sous \textsf{Windows} vous
  devriez avoir quelque chose de la forme 
  \verb|C:/Documents and Settings/| \ldots}~:
...
compliqué. Vous pouvez alors utiliser la fonction \rfunc{selectwd} de
l'extension \rgrs\footnote{Sous \textsf{Windows}, si vous utilisez
  l'interface graphique par défaut, vous pouvez aussi utiliser la
  fonction \textit{Changer le répertoire courant} du menu
  \textit{Fichier} } \margrs en tapant simplement~:

<<eval=FALSE>>=
selectwd()
@  %def

Une boîte de dialogue devrait alors s'afficher vous permettant de
sélectionner un répertoire sur votre disque. Sous \textsf{Windows}
elle devrait ressembler à celle de la figure~\ref{fig_selectwd}
\vpageref{fig_selectwd}.

\begin{figure}
  \begin{center}
    \includegraphics[width=5cm]{img/selectwd.png}
  \end{center}
  \caption{Sélection du répertoire de travail avec \rfunc{selectwd}}
  \label{fig_selectwd}
\end{figure}

Sélectionnez le répertoire de travail de votre session \R est cliquez
sur \textit{Ok}\footnote{Sous \textsf{Windows}, si vous ne retrouvez
  pas votre répertoire \textit{Mes documents}, celui-ci se trouve en
  général dans le répertoire portant le nom de votre utilisateur situé
  dans le répertoire \textit{Documents and Settings} du lecteur
  \verb|{C:|. Par exemple
  \verb|C:\Documents and Settings\Administrateur\Mes Documents\|}.
...
Si vous travaillez en ligne de commande dans la console, le répertoire
de travail a été mis à jour. Si vous travaillez dans un script, il
peut être intéressant de rajouter la ligne \texttt{setwd} indiquée
précédemment au début de votre script pour automatiser cette
opération.
...
traitées avec un tableur du type \textsf{OpenOffice} ou
...
\begin{center}
  \includegraphics{img/donnees_tableur.png}
\end{center}
...
\begin{center}
  \includegraphics{img/selecteur_fichier_excel.png}
\end{center}
...
et de les enregistrer au format export ou dans un format texte. Cette
application est téléchargeable gratuitement, mais ne fonctionne que
sous \textsf{Windows}\footnote{Ou sous \textsf{Linux} et \textsf{Mac
    OS X} avec \textsf{wine}.}~:

\url{http://www.umass.edu/statdata/software/downloads/SASViewer/}
...
\subsection{\textsf{Modalisa}}

L'extension \rgrs fournit \margrs plusieurs fonctions pour l'import ou
l'export de données depuis ou vers \textsf{Modalisa} et pour leur
traitement, en particulier concernant les questions à réponses multiples.

L'import de données permet de récupérer des sauvegardes au format
ASCII et s'appuie sur la fonction \rfunc{mls.import}.

On trouvera davantage d'informations à l'adresse suivante~:

\url{http://alea.fr.eu.org/j/rgrs_modalisa.html}
...
\item \margrs \rfunc{mls.export}, de l'extension \rgrs, permet
  d'exporter des données à destination de \textsf{Modalisa}~;
...
l'enquête \textit{Histoire de vie}, fourni avec l'extension \rgrs et
...
utiliser la fonction \rfunc{renomme.variable} de \margrs l'extension
\rgrs. Celle-ci prend en argument le tableau de données, le nom actuel
...
@ 
<<>>=
d <- renomme.variable(d, "bricol", "bricolage")
table(d$bricolage)
@ %def 

@ 
<<echo=FALSE,results=hide>>=
d <- renomme.variable(d, "bricolage", "bricol")
@ %def 
...
Par défaut, les valeurs manquantes ne sont pas considérées comme un
niveau de facteur. On peut cependant les transformer en niveau en
utilisant l'option \texttt{exclude=NULL}. Ceci signifie cependant
qu'elle ne seront plus considérées comme manquantes par \R~:

@ 
<<>>=
summary(d$trav.satisf)
summary(factor(d$trav.satisf, exclude=NULL))
@ %def 
...
Par contre il n'est pas possible d'utiliser directement
l'opérateur <<~\texttt{-}~>> comme pour l'indexation directe. On doit
effectuer une pirouette un peu compliquée utilisant la fonction
\rfunc{which}. Celle-ci renvoit les positions des éléments
satisfaisant une condition. On peut ainsi faire~:

@ 
<<>>=
which(names(d)=="qualif")
d2 <- d[,-which(names(d)=="qualif")]
@ %def 
...
On voit que le test \texttt{NA > 3} ne renvoit ni vrai ni faux, mais
\texttt{NA}.
...
pour l'indexation, lorsque la condition renvoit \texttt{NA}, l'élément
est sélectionné, comme s'il valait \texttt{TRUE}. Ceci aura donc des
...
fait que celle-ci est évaluée en \texttt{NA} et au final sélectionnée
par l'indexation~:
...
La conversion d'un facteur en caractères est fréquemment utilisé lors
des recodages du fait qu'il est impossible d'ajouter de nouvelles
modalités à un facteur. Par exemple, la première des commandes
suivantes génère un message d'avertissement, tandis que les deux
autres fonctionnent~:
...
Enfin, l'extension \rgrs \margrs propose une fonction
...
recodage ont parfois une influence dans le résultat final.
...
renvoit pas les valeurs du vecteur triées, mais les emplacements de
ces valeurs.
...
Ce que cette fonction renvoit, c'est l'ordre dans lequel on doit
...
Une manière d'organiser notre script\footnote{Ceci n'est qu'une
  ...
  évidemment très hautement subjectif.} pourrait être de placer les
...
Dés lors, si nous rajoutons les appels à la fonction \rfunc{source}
...
fournis avec l'extension \rgrs \margrs et tiré de l'enquête
...
comparaisons. L'extension \rgrs \margrs fournit des fonctions
...
\rfunc{print} (pour affichage dans \R) ou \rfunc{copie} (pour export
...
$\chi^2$\footnote{On ne donnera pas plus d'indications sur le test du
  ...
  suivante~:
  \url{http://alea.fr.eu.org/j/test_khi2.html}.}. Celui-ci
...
la fonction \rfunc{residus} \margrs de \rgrs~:
...
\section{Fonctions de l'extension \rgrs}

L'extension \rgrs propose quelques fonctions permettant de
...
\item[\texttt{normwt}] si \texttt{TRUE}, on normalisate les poids pour
...
Les fonctions \rfunc{lprop}, \rfunc{cprop} et \rfunc{residus} de \rgrs
\margrs sont donc tout à fait compatibles avec l'utilisation de
...
\begin{itemize}
\item utiliser les options de pondération des fonctions
  usuelles ou les fonctions de l'extension \textsf{rgrs} pour les cas
  les plus simples~;
...
\item surtout ne jamais modifier les variables du
  ...
  les données pondérées, puis redéclarer le \textit{design} pour que
...
Cette partie aborde l'utilisation de \R pour la création de cartes
simples permettant la représentation d'effectifs, de proportions ou de
variables qualitatives pour des zonages géographiques (régions,
communes, Iris\ldots). Ceci ne constitue qu'une infime partie des
possibilités de l'analyse spatiale de données.

Par ailleurs, le parti pris est ici de tout effectuer à l'intérieur de
\R, sans faire appel à des applications externes spécialisées comme
\textsf{Quantum GIS}, \textsf{gvSig} ou \textsf{GRASS}. 

Les fonctions présentées ici font partie pour la plupart de
l'extension \rgrs \margrs, mais elles ne sont que des interfaces
visant à faciliter l'utilisation de fonctions disponibles dans des
extensions spécialisées, en particulier l'extension \textsf{sp}.

\section{Données spatiales}

Sous \R, une carte est un objet comme un autre, seulement un peu plus
compliqué. Le stockage des données utilisé dans cette partie repose
sur la classe d'objets nommée \texttt{SpatialPolygonsDataFrame},
définie par l'extension \textsf{sp}. Ce type d'objet, particulièrement
complexe, peut contenir à la fois des données de type spatial (sous la
forme d'une liste de polygones) et des données classiques sous la
forme d'un tableau de données.

\subsection{Exemple d'objet spatial}

L'extension \rgrs \margrs fournit un exemple d'objet de ce type, nommé
\texttt{lyon}, et qui contient le contour des 9 arrondissements de
cette commune. On peut le charger dans \R de la manière suivante~:
...
Nous avons désormais à notre disposition un objet nommé \texttt{lyon}
que nous pouvons tout de suite représenter graphiquement à l'aide de
la fonction \texttt{plot}. Le résultat est indiqué
figure~\ref{fig_plotlyon} \vpageref{fig_plotlyon}.
...
@ %def 
  \end{center}
  \caption{\texttt{plot} d'un objet de type spatial}
  \label{fig_plotlyon}
\end{figure}

Si l'on veut étudier la structure de l'objet \texttt{lyon}, par
exemple en effectuant un \texttt{str(lyon)}, on se rend vite compte de
la complexité de ce type d'objets. En fait \texttt{lyon} est lui-même
composé de plusieurs <<~sous-objets~>> (\textit{slots}) accessibles
avec l'opérateur \texttt{@}. Nous décrirons les trois principaux~:

\begin{description}
  \item[\texttt{lyon@data}] est un tableau de données dont chaque
    ligne correspond à un des polygones (c'est à dire ici à un
    arrondissement de Lyon) et qui lui associe un certain nombre de
    données (identifiant, nom de l'arrondissement, etc.)~;
  \item[\texttt{lyon@polygons}] est une liste de polygones définissant
    les contours de chaque arrondissement~;
  \item[\texttt{proj4string}] est un objet décrivant la projection
    géographique utilisée pour les données spatiales.
\end{description}

On n'accèdera en général jamais directement à ces sous-objets, à part
éventuellement \texttt{data}, que l'on peut manipuler comme n'importe
quel tableau de données~:
...
\subsection{Importer des données spatiales}

\begin{remarque}
  Cette section est relativement technique et peut-être sautée si l'on
  souhaite juste avoir un aperçu des fonctionnalités présentées en
  utilisant les données incluses dans \rgrs.
\end{remarque}

La conversion et l'import de données de type spatial sont des
opérations relativement complexes, notamment du fait qu'elles mettent
en jeu des notions de projection pas toujours faciles à comprendre et
à maîtriser pour des non-spécialistes\footnote{Et en premier lieu par
  l'auteur de ces lignes.}.

\R propose cependant de nombreux outils pour importer des données de
différents formats, notamment \textit{via} les extensions
\textsf{maptools} et \textsf{rgdal}.

On n'entrera pas ici dans le détail de ces opérations (que nous ne
maîtrisons guère). Mais voici cependant, à titre indicatif, la marche
à suivre pour importer dans \R des données de l'IGN telles que
fournies par le Centre Maurice Halbwachs (contours Iris de différents
départements par exemple).

Les données étant fournies au format \textsf{MapInfo}, la première
opération consiste à les convertir en format \textsf{ESRI
  Shapefile}. Sous \textsf{Linux} cela se fait très facilement grâce à
\texttt{ogr2ogr} avec une commande du type~:

\begin{verbatim}
   ogr2ogr -f "ESRI Shapefile" 69_iris.shp 69_iris.mid
\end{verbatim}

Ceci devrait vous générer trois fichiers portant le même nom mais avec
les extensions \texttt{shp}, \texttt{shx} et \texttt{dbf}. L'import
dans \R peut alors s'effectuer de la manière suivante~:
...
On peut ensuite charger le contenu du fichier \texttt{rhone\_iris.rda}
à l'aide de la fonction \rfunc{load} dans un autre script.


\section{Cartes simples}

Dans ce qui suit on se base sur la carte des arrondissements de Lyon
et sur l'extrait du recensement 1999 pour les communes du Rhône. Ces
données peuvent être chargées avec les commandes suivantes~:
...
\subsection{Représentation de proportions}

Nous disposons donc, d'un côté, d'un objet spatial représentant les
arrondissements de Lyon, et de l'autre d'un tableau de données
contenant un extrait du recensement de 1999 pour les communes du
Rhône. Si on regarde un peu plus attentivement la structure de ces
deux objets~:
...
On se rend compte que les deux objets peuvent être <<~joints~>> grâce
à un champ contenant le code INSEE de chaque commune ou
arrondissement. Ce champ se nomme \texttt{DepCom} pour l'objet
\texttt{lyon}, et \texttt{code} pour l'objet \texttt{rp99}.

Nous avons dès lors tout ce qu'il nous faut pour afficher une première
carte, en l'occurrence le taux de chômage par arrondissement en
1999. On utilise pour cela la fonction \rfunc{carte.prop} de \rgrs. Le
code et le résultat sont indiqués figure~\ref{fig_propchom1} \vpageref{fig_propchom1}.
...
@ %def 
  \end{center}
  \caption{Exemple d'utilisation de \rfunc{carte.prop}}
  \label{fig_propchom1}
\end{figure}

La fonction \rfunc{carte.prop} admet les arguments suivants~:
\begin{itemize}
\item le premier argument est l'objet de type spatial contenant les
  données cartographiques, ici \texttt{lyon}~;
\item le second argument est le tableau de données contenant les
  variables à cartographier, ici \texttt{rp99}~;
\item le troisième argument est le nom de la variable à représenter,
  ici \texttt{"tx.chom"}~;
\item l'argument \texttt{sp.key} correspond au nom du champ de
  jointure dans l'objet spatial~;
\item l'argument \texttt{data.key} correspond au nom du champ de
  jointure dans le tableau de données.
\end{itemize}

Le nombre de classes de valeurs et les limites de ces classes sont
calculés automatiquement. On peut cependant spécifier soit un nombre
de classes à l'aide de l'argument \texttt{nbcuts}\footnote{Du fait que
  la fonction essaye d'établir des limites de classes <<~propres~>>,
  correpsondant par exemple à des nombres entiers, le nombre de
  classes indiqué n'est pas toujours respecté.}, soit les limites de
ces classes avec l'argument \texttt{at}, comme indiqué
figure~\ref{fig_propchom2} \vpageref{fig_propchom2}, et
figure~\ref{fig_propchom3} \vpageref{fig_propchom3}.
...
@ 
<<fig=TRUE>>=
carte.prop(lyon, rp99, "tx.chom", sp.key="DepCom", data.key="code", nbcuts=3)
@ %def 
  \end{center}
  \caption{Utilisation de l'argument \texttt{nbcuts} de \rfunc{carte.prop}}
  \label{fig_propchom2}
\end{figure}
...
@ 
<<fig=TRUE>>=
carte.prop(lyon, rp99, "tx.chom", sp.key="DepCom", data.key="code", at=c(10,10.5,11,11.5,12,15))
@ %def 
  \end{center}
  \caption{Utilisation de l'argument \texttt{at} de \rfunc{carte.prop}}
  \label{fig_propchom3}
\end{figure}

De nombreuses options sont disponibles pour personnaliser l'affichage
de la carte. On pourra citer~:

\begin{description}
\item[\texttt{main}] titre de la carte~;
\item[\texttt{sub}] sous-titre de la carte~;
\item[\texttt{posleg}] position de la légende, spécifiée sous forme de
  d'une chaîne de caractères~: \texttt{bottomright}, \texttt{topleft},
  \texttt{left}, \texttt{center}, \ldots~;  
\item[\texttt{diverg}] si \texttt{TRUE}, indique que la carte représente à la
  fois des valeurs négatives et positives~;
\item[\texttt{palette.pos}] palette de couleur utilisée pour les
  valeurs positives~;
\item[\texttt{palette.neg}] palette de couleur utilisée pour les
  valeurs négatives~;
\item[\texttt{palette}] palette de couleur spécifiée manuellement~;
\end{description}

Enfin, tout argument supplémentaire est transmis à la fonction
\rfunc{spplot} pour l'affichage de la carte.

Les palettes utilisées en argument de \texttt{palette.pos} et
\texttt{palette.neg} sont celles définies par l'extension
\textsf{RColorBrewer}, elle-même issue du projet \textit{Colorbrewer}~:

\url{http://www.colorbrewer.org}

Le site du projet propose notamment un outil qui permet de visualiser
et de choisir une palette de manière interactive~:

\url{http://www.personal.psu.edu/cab38/ColorBrewer/ColorBrewer.html}

Les noms de palettes passés en argument de \texttt{palette.pos} et
\texttt{palette.neg} sont les mêmes que ceux utilisés sur ce site.

Un exemple d'utilisation de ces différents paramètres est donné
figure~\ref{fig_propchom4} \vpageref{fig_propchom4}.
...
@ 
<<fig=TRUE>>=
carte.prop(lyon, rp99, "tx.chom", sp.key="DepCom", data.key="code", main="Taux de chômage1999", sub="Source : INSEE, RP 1999", palette.pos="RdPu", posleg="topright")
@ %def 
  \end{center}
  \caption{Personnalisations de l'affichage de \rfunc{carte.prop}}
  \label{fig_propchom4}
\end{figure}

\subsection{Représentation d'effectifs}

Contrairement à la reprséentation de proportions, qui s'effectue en
affectant une couleur à chaque polygone, les effectifs ou les
populations sont en général représentés par un symbole de taille
variable. Ce type de carte peut être obtenue avec la fonction \rfunc{carte.eff}.

Son utilisation est très semblable à celle de \rfunc{carte.prop} pour
ses arguments principaux. Un exemple utilisant les options par défaut
et représentant la population active de chaque arrondissement est
donné figure~\ref{fig_effact1} \vpageref{fig_effact1}.
...
@ %def 
  \end{center}
  \caption{Exemple d'utilisation de \rfunc{carte.eff}}
  \label{fig_effact1}
\end{figure}

Les options \texttt{nbcuts} et \texttt{at} sont disponibles de la même
manière que pour \rfunc{carte.prop}, mais elles n'agissent ici que sur
la présentation de la légende.

D'autres options de personnalisation sont également disponibles. On
retrouve les options \texttt{main}, \texttt{sub} et \texttt{posleg}
déjà décrites pour \rfunc{carte.prop}, ainsi que les options suivantes~:

\begin{description}
\item[\texttt{col.bg}] couleur des symboles (rouge par défaut)~;
\item[\texttt{col.border}] couleur de la bordure des symboles (blanc
  par défaut)~;
\item[\texttt{cex}] facteur d'agrandissement des symboles~;
\item[\texttt{pch}] symbole utilisé~;
\item[\texttt{plot.polygons}] si \texttt{FALSE}, on n'affiche que les symboles
  et pas les polygones. 
\end{description}

Un exemple d'utilisation de ces différents paramètres est donné
figure~\ref{fig_effact2} \vpageref{fig_effact2}.
...
@ %def 
  \end{center}
  \caption{Personnalisations de l'affichage de \rfunc{carte.eff}}
  \label{fig_effact2}
\end{figure}


\subsection{Représentation d'une variable qualitative}

La représentation d'une variable qualitative (typiquement le résultat
d'une classification) se fait généralement de la même manière que la
représentation d'une proportion, mais en utilisant une palette de
couleurs contrastées et n'induisant pas de <<~hiérarchie~>> entre les
objets représentés. Ce type de carte peut être obtenue avec la
fonction \rfunc{carte.qual}.

Là encore, son utilisation est très semblable à celle de
\rfunc{carte.prop} et \rfunc{carte.eff}. Un exemple utilisant les
options par défaut et représentant la population active de chaque
arrondissement est donné figure~\ref{fig_qual1}
\vpageref{fig_qual1}. Le tableau de données \texttt{rp99} ne
contenant pas de variable de type qualitative, on a simulé les données
...
carte.qual(lyon, rp99, "qual", sp.key="DepCom", data.key="code")
@ %def 
  \end{center}
  \caption{Exemple d'utilisation de \rfunc{carte.qual}}
  \label{fig_qual1}
\end{figure}

Là encore, plusieurs options sont disponibles pour personnaliser
l'affichage de la carte. Ce sont en fait les mêmes options que celles
de \rfunc{carte.prop} (\texttt{main}, \texttt{sub}, \texttt{posleg},
\texttt{palette}\ldots), ainsi que l'option \texttt{palette.qual}, qui
permet de sélectionner une palette de l'extension
\texttt{RColorBrewer}. Un exemple de personnalisation est donné
figure~\ref{fig_qual2} \vpageref{fig_qual2}.
...
@ 
<<fig=TRUE>>=
carte.qual(lyon, rp99, "qual", sp.key="DepCom", data.key="code", main="Catégories d'arrondissements", sub="Source : aléatoire", posleg="bottomright", palette.qual="Set2")
@ %def 
  \end{center}
  \caption{Exemple de personnalisation de \rfunc{carte.qual}}
  \label{fig_qual2}
\end{figure}


\section{Ajout d'éléments à une carte}

Il est tout à fait possible de superposer d'autres éléments graphiques
à une carte. On utilise alors en général directement la fonction
\rfunc{plot} munie de l'argument \texttt{add=TRUE}.

\subsection{Bordure}

Il peut être intéressant d'ajouter une bordure à une carte soit pour
des raisons esthétiques (par exemple pour délimiter son contour), soit
pour mettre en valeur une zone particulière.

Imaginons par exemple que nous souhaitons mettre en valeur le premier
arrondissement de Lyon sur notre carte. Un objet de type spatial,
malgré sa complexité, supporte l'indexation d'une manière semblable
aux tableaux de données. On peut donc sélectionner un sous-ensemble de
notre carte des arrondissements de Lyon de la manière suivante~:

@ 
<<>>=
lyon1 <- lyon[lyon@data$Nom_Com == "LYON 1ER",]
@ %def 

Cette commande sélectionne le polygone dont le nom est \texttt{LYON
  1ER} et le place dans un nouvel objet. Il est alors très simple de
rajouter une bordure autour de cet arrondissement en utilisant la
fonction \rfunc{plot}, comme indiqué figure~\ref{fig_cartebord1}
\vpageref{fig_cartebord1}.
...
lyon1 <- lyon[lyon@data$Nom_Com == "LYON 1ER",]
plot(lyon1, lwd=5,border="red",add=TRUE)
@ %def 
  \end{center}
  \caption{Exemple d'ajout d'une bordure autour d'une zone}
  \label{fig_cartebord1}
\end{figure}

Si on souhaite délimiter le contour de notre carte avec une bordure,
on doit effectuer une opération supplémentaire, qui consiste à
<<~fusionner~>> l'ensemble des polygones de notre carte pour n'en
garder que le contour. Cette opération peut se faire dans \R à l'aide
de la fonction \rfunc{unionSpatialPolygons} de l'extension
\textsf{maptools}. Celle-ci permet de fusionner les polygones d'une
carte en fonction des valeurs d'une variable~: les polygones ayant la
même valeur sont alors regroupés pour n'en former plus qu'un.

Dans notre cas, nous voulons fusionner tous les polygones, nous
pouvons donc créer une variable artificielle (ici nommée
\texttt{fusion}) contenant la même valeur pour toutes nos zones, puis
appliquer la fonction \rfunc{unionSpatialPolygons}, ce qui donne~:

@ 
<<results=hide>>=
library(maptools)
fusion <- rep(1, nrow(lyon@data))
lyon.contour <- unionSpatialPolygons(lyon, fusion)
@ %def 

On peut ensuite utiliser l'objet ainsi calculé pour ajouter une
bordure globale à notre carte, comme dans la figure~\ref{fig_cartebord2}
\vpageref{fig_cartebord2}.
...
carte.prop(lyon, rp99, "tx.chom", sp.key="DepCom", data.key="code", main="Taux de chômage 1999")
plot(lyon1, lwd=5,border="red",add=TRUE)
plot(lyon.contour, lwd=3,border="black",add=TRUE)
@ %def 
  \end{center}
  \caption{Exemple d'ajout d'une bordure globale} 
  \label{fig_cartebord2}
\end{figure}


\subsection{Labels}

L'ajout de labels est souvent indispensable pour augmenter la
lisibilité d'une carte et permettre le repérage des zonages
géographiques représentés. La fonction \rfunc{carte.labels} est faite
pour cela.

Elle accepte comme principaux arguments~:

\begin{itemize}
\item le nom de l'objet spatial~;
\item un vecteur de chaînes de caractères contenant les labels~;
\end{itemize}

On peut spécifier les coordonnées de placement des labels via
l'argument \texttt{coords}. Si cet argument vaut \texttt{NULL} (ce qui
est le cas par défaut), la position des labels est automatiquement
calculée en fonction du polygone auquel il appartient (ce qui n'exclut
cependant pas les chevauchements).

La figure~\ref{fig_cartelabels1} \vpageref{fig_cartelabels1} montre
comment ajouter les noms des arrondissements à notre carte de Lyon.
...
<<fig=TRUE>>=
carte.prop(lyon, rp99, "tx.chom", sp.key="DepCom", data.key="code", main="Taux de chômage 1999")
carte.labels(lyon, lyon@data$Nom_Com)
@ %def 
  \end{center}
  \caption{Exemple d'ajout de labels} 
  \label{fig_cartelabels1}
\end{figure}

Plusieurs options sont disponibles pour personnaliser l'affichage des
labels, notamment~:

\begin{description}
\item[\texttt{cex}] facteur d'agrandissement~;
\item[\texttt{font}] style de police de caractère (gras par
  défaut). Voir la page d'aide de \texttt{par} pour plus de détails~;
\item[\texttt{col}] couleur du texte~;
\item[\texttt{outline, outline.decal, outline.col}] permettent
  d'ajouter une <<~bordure~>> autour des labels.
\end{description}

La figure~\ref{fig_cartelabels2} \vpageref{fig_cartelabels2} montre
comment ajouter la valeur du taux de chômage à notre carte, en leur
...
<<fig=TRUE>>=
carte.prop(lyon, rp99, "tx.chom", sp.key="DepCom", data.key="code", main="Taux de chômage 1999")
lyon.tx.chom <- round(rp99$tx.chom[rp99$code %in% lyon@data$DepCom],1)
carte.labels(lyon, lyon.tx.chom, outline=TRUE)
@ %def 
  \end{center}
  \caption{Exemple d'ajout de labels personnalisés} 
  \label{fig_cartelabels2}
\end{figure}
...
texte à l'aide de la fonction \rfunc{copie} \margrs de l'extension
\rgrs\footnote{Celle-ci nécessite que l'extension \texttt{R2HTML} soit
...
\rfunc{copie}. Celle-ci va alors transformer le tableau (ou l'objet)
...
\subsection{Export vers \textsf{Word} ou \textsf{OpenOffice} \textit{via} un fichier}

L'autre possibilité ne nécessite pas de passer par \textsf{Excel}, et
fonctionne sous \textsf{Word} et \textsf{OpenOffice} sur toutes les
plateformes.

Elle nécessite de passer à la fonction \rfunc{copie} l'option
...
existante à l'aide de la fonction \rfunc{dev.copy}. On doit fournir à
...
<<eval=FALSE>>=
boxplot(rnorm(100))
dev.copy(device=png, file="export.png")
dev.off()
@ %def 
...
\rfunc{postscript}, \rfunc{pdf}, \rfunc{svg}\footnote{Ne fonctionne
  pas sous \textsf{Windows}.} et \rfunc{win.metafile}\footnote{Ne
  fonctionne que sous \textsf{Windows}.} pour les formats vectoriels.
...
\section{Génération automatique de rapports avec \textsf{OpenOffice}}
...
\texttt{ODF}, donc en particulier avec \textsf{OpenOffice} mais pas
avec \textsf{Word}. L'utilisation d'\textsf{OpenOffice} est cependant
très proche de celle de \textsf{Word}, et les documents générés
peuvent être ensuite ouverts sous \textsf{Word} pour édition.
...
\url{http://alea.fr.eu.org/j/files/zip.zip}
...
rouge). Au final, le code \texttt{library(rgrs)} est exécuté, mais
...
\margrs \rfunc{genere.tableau}, qui, appliquée à un objet de type
...
\begin{description}
  ...
  \item[\texttt{results}] indique le type de résultat renvoyé par le
    ...
    fait appel à la fonction \rfunc{genere.tableau}. Si l'option vaut
...
généré par \rfunc{genere.tableau}, on utilise
...
La fonction \rfunc{genere.tableau} fait partie de l'extension \rgrs
\margrs. Elle transforme l'objet qu'on lui passe en paramètres dans un
format mis en forme lisible par \textsf{OpenOffice}\footnote{En fait
  la fonction \rfunc{genere.tableau} ne fait rien par elle-même, elle
se contente de simplifier l'appel à la fonction \rfunc{odfTable} de
l'extension \textsf{odfWeave}.}.

Elle permet de transformer les objets de type suivant~:

\begin{itemize}
\item \texttt{table} à une ou deux dimensions (tri à plat ou tableau
  croisé obtenu avec les fonctions \rfunc{table}, \rfunc{lprop},
  \rfunc{cprop}\ldots
\item \texttt{data.frame}, y compris les tableaux de données ou le
  résultat de la commande \rfunc{freq}.
\item \texttt{vector}
\item \texttt{matrix}
\end{itemize}
...
\section{Génération automatique de rapports avec \LaTeX}

Des fonctionnalités similaires à celles offertes par l'extension
\textsf{odfWeave} sont fournies pour \LaTeX{} par l'extension
\textsf{Sweave}, permettant de générer dynamiquement des documents
contenant du code \R avec un rendu typographiquement de haute qualité.

Comme cette extension nécessite l'apprentissage de \LaTeX, elle
dépasse le cadre de ce document\footnote{C'est cet outil qui a permis
  de générer le document que vous avez sous les yeux.}. 
...
\item[Details] apporte des précisions sur le fonctionnement interne de
  la fonction
\item[Note] pour des remarques éventuelles
\item[References] pour des références bibliographiques ou des URL associées
\item[See Also] \textit{très utile}, renvoit vers d'autres fonctions
...
\rfunc{help.search}, qui renvoit une liste des pages d'aide contenant
...
toujours très actif des messageries instantanées actuelles. Deux
canaux sont dédiés aux échanges autour de \R, l'un en anglais
(\texttt{\#R}) et l'autre en français (\texttt{\#Rfr}).
...
Dans le champ \textit{Connect to IRC}, sélectionnez
\textit{Freenode.net}, puis saisissez un pseudonyme dans le champ
\textit{Nick} et le nom du canal dans le champ \textit{Channel}
(\texttt{\#R} ou \texttt{\#Rfr}). Vous pourrez alors discuter
directement avec les personnes présentes.

Les deux canaux IRC dédiés à \R sont normalement peuplés de personnes
qui seront très heureuses de répondre à toutes les questions, et en
général l'ambiance y est très bonne. Une fois votre question posée,
n'hésitez pas à être patient et à attendre quelques minutes, voire
quelques heures, le temps qu'un des habitués vienne y faire un
tour. Le canal \texttt{\#Rfr}, en particulier, n'est pour le moment
pas extrêmement actif.
...
\url{http://cran.cict.fr/bin/windows/base/release.htm}

Vous allez alors vous voir proposer le téléchargement d'un fichier
nommé \texttt{R-2.X.X-win32.exe} (les \texttt{X} étant remplacés par
...
Enfin signalons l'existence, grâce au travail de Mayeul Kauffmann,
d'une version portable de \R pour \textsf{Windows} permettant
notamment d'installer facilement sur une clé USB \R, l'éditeur
\textsf{Tinn-R} ainsi que plusieurs extensions. On peut donc avoir en
permanence sur soi un environnement de travail complet~:

\url{http://otan.ecoledelapaix.org/spip.php?article102}


\section{Installation de \R sous \textsf{Mac OS X}}

\R fonctionne pour les versions de \textsf{Mac OS X} 10.2
ultérieures. Néanmoins l'installateur par défaut nécessite au minimum
une version 10.4.4 (\textit{Tiger}).

L'installation est très simple~:

\begin{enumerate}
\item Se rendre à la page suivante~: \url{http://cran.r-project.org/bin/macosx/}
\item Télécharger le fichier nommé \texttt{R-2.X.Y.dmg}
\item Double cliquer sur le fichier téléchargé. Une fenêtre devrait
  s'ouvrir, contenant le programme d'installation.
\item Il vous suffit alors de double cliquer sur le programme
...
\item Se rendre dans le répertoire d'installation de \R, en général
  ...
  installation. En clair, si vous mettez à jour de \R 2.6.2 vers \R
  2.7.1, copiez tout le contenu du répertoire
  \verb|C:\Program Files\R\R-2.6.2\library| dans
  \verb|C:\Program Files\R\R-2.7.1\library|.
...
L'une des alternatives les plus simples sous \textsf{Windows} est sans
doute l'éditeur de texte \textsf{Tinn-R}. Son installation est décrite
section~\pageref{sec_tinnr}~\vpageref{sec_tinnr}.


La version de \R pour \textsf{Mac OS X} est fournie avec une interface
graphique beaucoup plus ergonomique et comprenant un éditeur de texte
assez complet. Sous Linux et Unix, plusieurs alternatives
existent. Pour ceux qui maîtrisent \textsf{Emacs}, le mode
\textsf{ESS} comprend une interface complète pour \R. Pour des
interfaces plus graphiques, on pourra aussi jeter un \oe{}il du côté
du projet \textsf{Rkward}~:

\url{http://rkward.sourceforge.net/?content=home&lang=fr}
...
\subsection{\textsf{Tinn-R}}
\label{sec_tinnr}

\textsf{Tinn-R} est une alternative à l'interface graphique installée
par défaut avec \R sous \textsf{Windows}, et ne fonctionne que sur ce
système. Il s'agit d'un éditeur de texte, mais qui propose la
coloration syntaxique des scripts \R, la soumission de commandes
directement depuis l'éditeur, et une aide en ligne efficace. Son
interface n'est cependant disponible qu'en anglais.

Il s'agit d'un logiciel libre et gratuit. Son installation est
vivement recommandée en cas d'utilisation régulière de \R sous
\textsf{Windows}.
...
Le téléchargement peut s'effectuer directement depuis \textit{Sourceforge}~:

\url{http://sourceforge.net/project/platformdownload.php?group_id=144024}

en sélectionnant le fichier nommé \texttt{Tinn-R\_2.1.1.6\_setup.exe}
(ou quelque chose y ressemblant). Une fois le fichier téléchargé il
suffit de l'exécuter et de poursuivre l'installation en laissant les
options par défaut.

\begin{important}
Il est nécessaire d'avoir installé \R sur votre système \textit{avant}
d'installer \textsf{Tinn-R}. De même, si vous aviez déjà une version
de \textsf{Tinn-R} plus ancienne sur votre système, il est conseillé
de la désinstaller avant d'installer la nouvelle. Dans ce cas, si vous
souhaitez conserver votre configuration, vous pouvez utiliser les
outils \textit{Backup} et \textit{Restore} de menu \textit{Tools}.
\end{important}

Une fois \textsf{Tinn-R} installé, vous pouvez le lancer \textit{via}
le menu \textit{Démarrer}. Plusieurs étapes de configuration restent à
effectuer avant de pouvoir réellement commencer à travailler.

Tout d'abord, sélectionnez le menu \textit{R}, puis
\textit{Customize}, puis \textit{Rconfigure.r}. Un nouveau fichier
apparaît. Ajoutez alors les ligness suivantes à un endroit quelconque,
comme indiqué figure~\ref{tinnr_config} \vpageref{tinnr_config}~:
...
\begin{figure}
  \begin{center}
    \includegraphics{img/tinnr_config.png}
  \end{center}
  \caption{Configuration de \textsf{Tinn-R}}
  \label{tinnr_config}
\end{figure}


Enregistrez le fichier (\textit{File} puis \textit{Save} ou bien
\texttt{Ctrl+S}) et fermez sa fenêtre (\textit{File} puis
\textit{Close} ou \texttt{Ctrl+W}).

Sélectionnez ensuite le menu \textit{R}, puis \textit{Configure}, puis
\textit{Permanent}. Un nouveau fichier devrait s'ouvrir, et une boîte
de dialogue s'afficher en vous proposant de lancer \R. Choisissez
\textit{Non}.

Sélectionnez ensuite le menu \textit{R}, \textit{Start/close and
  connections}, puis \textit{Rterm}. Vous devriez voir une nouvelle
fenêtre apparaître dans laquelle devrait se lancer \R. Vous pouvez
désormais utiliser la fenêtre \R comme la console dans l'interface par
défaut, saisir directement des commandes et naviguer dans l'historique
à l'aide des touches \texttt{Alt+Flèche haut} et \texttt{Alt+Flèche
  bas}.

Vous pouvez également créer un nouveau script en choisissant
\textit{File} puis \textit{New}. Si vous saisissez des commandes \R
dans ce fichier, vous pouvez les exécuter directement en choisissant
le menu \textit{R}, puis \textit{Send} puis soit \textit{Line},
\textit{Selection}, \textit{File}\ldots ou en utilisant les icônes de
la barre d'outil équivalentes.

À noter que par défaut la console \R de \textsf{Tinn-R} comporte deux
onglets, l'un nommé \texttt{IO} et l'autre \texttt{Log}. C'est dans ce
deuxième onglet que s'affichent les messages d'erreur et
d'avertissement (par défaut un bip est émis lorsqu'une erreur
survient). Ce comportement par défaut n'étant pas pratique du tout,
vous pouvez le modifier en choisissant le menu \textit{R}, puis
\textit{Rterm}, puis \textit{Split} et enfin \textit{Horizontal
  split}. Au final, vous devriez obtenir une interface ressemblant
quelque peu à la figure~\ref{tinnr_interface}
\vpageref{tinnr_interface}.

\begin{figure}
  \begin{center}
    \includegraphics{img/tinnr_interface.png}
  \end{center}
  \caption{Interface de \textsf{Tinn-R}}
  \label{tinnr_interface}
\end{figure}

De nombreuses options de configuration existent. Les principales se
trouvent dans le menu \textit{Options}, \textit{Application} sous
l'onglet \textit{R}. Mais les options les plus importantes sont sans
doute celles permettant de modifier les raccourcis claviers pour
envoyer du code à \R depuis un script. 

Par exemple, si vous souhaitez pouvoir exécuter une ligne en utilisant
\texttt{Ctrl+Entrée}~:
\begin{enumerate}
\item Sélectionnez le menu \textit{R} puis \textit{Hotkeys}.
\item Une boîte de dialogue s'ouvre. Dans le champ \textit{Sending and
  controlling}, sélectionnez la ligne \textit{Send: line}.
\item Dans le champ \textit{Set (hotkey method)}, cochez la case
  \texttt{Ctrl} et choisissez \texttt{ENTRÉE} dans la liste
  déroulante, puis cliquez sur \textit{Add}.
\item Vérifiez qu'en bas de la boîte de dialogue le champ
  \textit{Option} est bien positionné sur \textit{Active}. au final
  vous devriez avoir quelque chose ressemblant à la
  figure~\ref{tinnr_raccourci} \vpageref{tinnr_raccourci}.
\item Cliquez sur \texttt{OK}.
\end{enumerate}

\begin{figure}
  \begin{center}
    \includegraphics{img/tinnr_raccourci.png}
  \end{center}
  \caption{Création d'un raccourci clavier dans \textsf{Tinn-R}}
  \label{tinnr_raccourci}
\end{figure}


Vous devriez maintenant pouvoir exécuter une ligne directement depuis
votre script en utilisant \texttt{Ctrl+Entrée}. Vous pouvez faire de
même pour \textit{Send: selection} ou \textit{Send: file}\ldots

De nombreuses autres fonctionnalités sont disponibles dans
\textsf{Tinn-R}, comme l'aide en ligne, l'affichage et la manipulation
des objets en mémoire, etc. Pour plus d'informations on pourra se
reporter à l'aide du logiciel, disponible \textit{via} le menu
\textit{Help}, et notamment l'entrée \textit{French},
\textit{Lisezmoi}, \textit{HTML}.
...
depuis lequel récupérer les données nécessaires. Choisissez de
préférence un miroir le plus proche possible de l'endroit où vous vous
trouvez\footnote{Ayant déjà rencontré des soucis avec le miroir
  lyonnais, j'ai tendance à utiliser celui de Toulouse.}.
...
\section{L'extension \rgrs}
\label{sec_rgrs}

\rgrs est une extension pour R comprenant quelques fonctions
potentiellement utiles pour l'utilisation du logiciel en sciences
sociales. Pour l'instant elle comporte essentiellement des fonctions
pour les tableaux croisés, l'export de résultats et pour le travail
avec des fichiers issus de Modalisa\footnote{À noter que les fonctions
  en question ne sont en général que des interfaces facilitant
  l'utilisation de fonctions déjà existantes.}.

\subsection{Installation}

L'installation nécessite d'avoir une connexion
active à Internet. Depuis la version 0.2-6, l'extension est hébergée
...
\begin{remarque}
  À noter que l'extension n'est disponible que pour les versions les
  plus récentes de \R. Ainsi, depuis la sortie de la version 2.8, elle
  n'est plus installable de la manière décrite précédemment pour les
  versions 2.7 de \R. Il est alors conseillé de mettre son
  installation de \R à jour.
\end{remarque}
...
utilisation, on pourra se reporter aux pages Web suivantes~:

\url{http://alea.fr.eu.org/j/rgrs.html}

Un document PDF regroupant les pages d'aide en ligne de l'extension
est notamment disponible~:

\url{http://cran.r-project.org/web/packages/rgrs/rgrs.pdf}

Ainsi qu'une page décrivant plus particulièrement l'utilisation des
fonctions facilitant l'importation et le traitement de données issues
de \textsf{Modalisa}~:
...
L'extension \rgrs contient plusieurs jeux de données
...
data(hdv2003)
d <- hdv2003
d <- renomme.variable(d, "clso", "classes.sociales")
d <- renomme.variable(d, "classes.sociales", "clso")
@ %def 
\end{solution}
