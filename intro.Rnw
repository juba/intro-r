...
\SweaveOpts{fig.path=tmp/,dev=tikz,sanitize=TRUE,cache=FALSE,cache.path=cache/,external=TRUE,prompt=TRUE,comment=NA,background=.9;.9;.9,fig.align=center,out.width=.8\textwidth,par=TRUE}

<<initialize,echo=FALSE,results=none>>=
options(width=85)
options(device = function(...) {
    .Call("R_GD_nullDevice", PACKAGE = "grDevices")
})
options(tikzDocumentDeclaration="\\documentclass[12pt,francais]{article}")
#options(tikzLatexPackages=c(
#	getOption("tikzLatexPackages"), "\\usepackage{sansmath}"
#))
