...
La colonne \texttt{n} donne les effectifs bruts, la colonne \texttt{\%} la
répartition en pourcentages, et la colonne \texttt{val\%} la répartition en
pourcentages une fois exclues les valeurs manquantes. 

La fonction accepte plusieurs paramètres permettant d'afficher les totaux, les
pourcentages cumulés, de trier selon les effectifs ou de contrôler l'affichage.
Par exemple~:
...
Les pourcentages lignes s'obtiennent avec la fonction \rfunc{lprop} de
l'extension \questionr \marqr. Celle-ci s'applique au tableau croisé
généré par \rfunc{table}~:
