...
\begin{rstudio}
  ...
  touche \textit{tabulation}. \textsd{RStudio} affichera la liste des fonctions
  ...
  \textit{tabulation} après avoir saisi \texttt{mean(t}) ou bien pour retrouver
  un argument nommé d'une fonction (par exemple, appuyez sur la touche 
  \textit{tabulation} après avoir saisi \texttt{mean(taille,}).
\end{rstudio}
