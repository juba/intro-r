...
s'appelle tout simplement \rfonc{save}.

Par exemple, si l'on souhaite sauvegarder son tableau de données \texttt{d} 
ainsi que les objets \texttt{tailles} et \texttt{poids} dans fichier 
\texttt{mon_export.RData} :

<<eval=FALSE>>=
save(d,tailles,poids,file="mon_export.RData")
@ 
