...
\textsf{RStudio} propose une interface pratique à \textsf{knitr}\footnotemark.
On peut ainsi facilement créer un fichier \textit{R Markdown}, \textit{R HTML}
ou \textit{R Sweave} et, d'un clic, générer des fichiers HTML pour les deux
premiers formats, ou PDF pour le dernier.
