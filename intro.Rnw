...
L'extension \questionr fournit \marqr plusieurs fonctions pour l'import ou
...
\item \marqr \rfunc{mls.export}, de l'extension \questionr, permet
...
utiliser la fonction \rfunc{renomme.variable} de \marqr l'extension
...
la fonction \rfunc{residus} \marqr de \questionr~:
...
Les fonctions \rfunc{lprop}, \rfunc{cprop} et \rfunc{residus} de \questionr
...
l'extension \questionr \marqr, mais elles ne sont que des interfaces
...
L'extension \questionr \marqr fournit un exemple d'objet de ce type, nommé
...
\begin{remarque}
  ...
  utilisant les données incluses dans \questionr.
\end{remarque}
...
1999. On utilise pour cela la fonction \rfunc{carte.prop} de \questionr. Le
...
texte à l'aide de la fonction \rfunc{copie} \marqr de l'extension
...
\marqr \rfunc{genere.tableau}, qui, appliquée à un objet de type
...
La fonction \rfunc{genere.tableau} fait partie de l'extension \questionr
\marqr. Elle transforme l'objet qu'on lui passe en paramètres dans un
