...
\date{Version 2.2 (OBSOLÈTE)\\\today{}\\\vspace{1cm}\textbf{Attention~:} ce document est désormais obsolète et ne sera plus mis à jour. Une version modernisée est disponible à l'adresse~: \url{https://juba.github.io/tidyverse/}\\\vspace{2cm}\includegraphics[width=9cm]{img/xkcd_tech_chart.png}\\\footnotesize\texttt{http://xkcd.com/627/}}
...
\begin{important}
Ce document est désormais obsolète et ne sera plus mis à jour. Une version modernisée est disponible à l'adresse~: \url{https://juba.github.io/tidyverse/}
\end{important}
...
\texttt{cut}, nommée \rfunc{icut}. Elle s'utilise soit dans RStudio via le
menu \textit{Addins}, puis \textit{Numeric range dividing}, soit depuis la console ou un script de la manière suivante~:
...
\section{Génération automatique de documents avec \textsf{RMarkdown}}
\label{sec-rmarkdown}

\textsf{R Markdown} est une extension \R, développée par RStudio, qui permet de
mélanger du code \R dans des documents au format \texttt{Markdown} et de produire en
...
\textsf{R Markdown} est extrêmement versatile. À partir d'un format de balisage léger, il permet d'inclure du code R et de
générer des documents dans différents formats, notamment HTML, PDF\footnote{Sous réserve d'avoir une installation fonctionnelle de \LaTeX{}} ou traitement de texte.
À noter que \textsf{RStudio}\footnote{Pour plus d'informations sur \textsf{RStudio}, voir
  section~\ref{sec_rstudio} \vpageref{sec_rstudio}.} propose une interface
pratique à \textsf{R Markdown}, mais qu'on peut aussi parfaitement l'utiliser en ligne de
...
\textit{File}, puis \textit{New file}, puis \textit{R Markdown...}. Une boîte dialogue vous permet alors d'indiquer un titre, un auteur et un format de sortie\footnote{Le format de sortie peut être modifié par la suite à tout moment}. Un nouveau document s'ouvre. Effacez son contenu et remplacez le par quelque chose comme~:
...
\begin{figure}
  ...
  \end{center}
  \caption{Résultat de la génération d'un document HTML par \textsf{R Markdown}}
...
Si vous n'utilisez pas \textsf{RStudio}, vous pouvez compiler votre fichier \textsf{R Markdown} en lançant R dans le même répertoire et en
...
précédée d'un dièse \texttt{\#} sera transformée en titre de niveau 1, etc.
...
Le bloc commence et se termine par trois quotes inverses suivies, entre
...
possibilités de \textsf{R Markdown}. Grâce à ce système, le code \R peut être
...
Le site \textit{France Université Numérique} propose plusieurs cours en français, dont un sur l'analyse de données multidimensionnelles :

\url{https://www.fun-mooc.fr/}

Le site \textit{Datacamp} propose une formation en ligne en français gratuite,
basée sur la pratique :

\url{https://www.datacamp.com/courses/introduction-a-r/}

En anglais, on trouvera des ressources sur \R sur les principales plateformes de MOOCs. On pourra citer par exemple les cours de la spécialisation \textit{Data science} sur Coursera :

\url{https://www.coursera.org/specializations/jhu-data-science}

Par ailleurs, \textit{Swirl} est une extension qui propose des cours <<~interactifs~>> directement intégrés à \R :
...
\url{https://groupes.renater.fr/sympa/subscribe/r-soc}
...
puis de rejoindre le canal en question.
...
Cependant, le projet \textsf{RStudio} tend à s'imposer comme l'environnement de
