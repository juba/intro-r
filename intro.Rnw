...
\SweaveOpts{fig.path=tmp/,dev=pdf,sanitize=TRUE,cache=TRUE,cache.path=cache/,external=TRUE,prompt=TRUE,comment=NA,background=.95;.95;.95,fig.align=center,out.width=.8\textwidth}
...
<<espaces,eval=FALSE,prompt=FALSE>>=  
R> 10+2
R> 10 + 2
R> 10       +       2
@ 
...
\section{Génération automatique de rapports avec \textsf{OpenOffice} ou \textsf{LibreOffice}}
...
\textsf{OpenDocument} (extension \texttt{.odf}), donc en particulier avec
...
\textsf{odfWeave} sont fournies pour \LaTeX{} par des extensions comme
\textsf{Sweave} ou \textsf{knitr}\footnote{C'est cet outil qui a permis de
  générer le document que vous avez sous les yeux.}, permettant de générer
dynamiquement des documents contenant du code \R avec un rendu typographique
de haute qualité.
